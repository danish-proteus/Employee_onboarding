...
function_name: validate_self_relation_gender
form_no: 4
field_name: relation
language: plpgsql
description: Validate gender/relation combination on change of relation
functional_specification: On change of RELATION (with GENDER already selected on the same family row): block disallowed combinations by returning an error - Female+Brother 'Brother cannot be female.', Female+Father 'Father cannot be female.', Female+GrandFather 'GrandFather cannot be female.', Female+Son 'Son cannot be female.', Female+Husband 'Husband cannot be female.', Male+Mother 'Mother cannot be male.', Male+Sister 'Sister cannot be male.', Male+GrandMother 'GrandMother cannot be male.', Male+Wife 'Wife cannot be male.', Male+Daughter 'Daughter cannot be male.'. If the combination is allowed, return no updates.
business_logic: Validate gender/relation combination on change of relation
*/

CREATE OR REPLACE FUNCTION validate_self_relation_gender(p jsonb) RETURNS jsonb LANGUAGE plpgsql AS $$
DECLARE
