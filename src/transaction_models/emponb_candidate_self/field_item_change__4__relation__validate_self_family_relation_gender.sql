...
function_name: validate_self_family_relation_gender
form_no: 4
field_name: relation
language: plpgsql
description: Validate gender-relation combination on relation change
functional_specification: On change of RELATION, check the current GENDER on the row. Block the combination (return an error / do not update) for: Female+Brother ('Brother cannot be female.'), Female+Father ('Father cannot be female.'), Female+GrandFather ('GrandFather cannot be female.'), Female+Son ('Son cannot be female.'), Female+Husband ('Husband cannot be female.'), Male+Mother ('Mother cannot be male.'), Male+Sister ('Sister cannot be male.'), Male+GrandMother ('GrandMother cannot be male.'), Male+Wife ('Wife cannot be male.'), Male+Daughter ('Daughter cannot be male.'). Return no updates when the combination is valid.
business_logic: Validate gender-relation combination on relation change
*/

CREATE OR REPLACE FUNCTION validate_self_family_relation_gender(p jsonb) RETURNS jsonb LANGUAGE plpgsql AS $$
DECLARE
