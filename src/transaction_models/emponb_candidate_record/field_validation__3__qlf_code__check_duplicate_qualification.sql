...
field_name: qlf_code
language: plpgsql
description: Block saving when the selected QLF_CODE is already present on another education line of the same candidate, checking both the unsaved sibling rows in the current transaction grid and the persisted EMPONB_CAND_EDUCATION rows.
business_logic: Block saving when the selected QLF_CODE is already present on another education line of the same candidate, checking both the unsaved sibling rows in the current transaction grid and the persisted EMPONB_CAND_EDUCATION rows.
*/
...
AS $$
DECLARE
    -- Current education row (form_no 3, table EMPONB_CAND_EDUCATION).
...
BEGIN
    ...
    -- (4) Duplicate found in either source => blocking validation error.
